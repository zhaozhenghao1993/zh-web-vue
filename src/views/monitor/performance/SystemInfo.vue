...
import { systemInfo } from '@/api/monitor/performance'
...
export default {
  ...
  methods: {
    ...
    loadTomcatInfo () {
      ...
      Promise.all([
        systemInfo('system.cpu.count'),
        systemInfo('system.cpu.usage'),
        systemInfo('process.start.time'),
        systemInfo('process.uptime'),
        systemInfo('process.cpu.usage')
      ]).then((res) => {
        const info = []
