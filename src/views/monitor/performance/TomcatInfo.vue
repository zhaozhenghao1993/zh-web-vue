...
import { tomcatInfo } from '@/api/monitor/performance'
...
export default {
  ...
  methods: {
    ...
    loadTomcatInfo () {
      ...
      Promise.all([
        tomcatInfo('tomcat.sessions.created'),
        tomcatInfo('tomcat.sessions.expired'),
        tomcatInfo('tomcat.sessions.active.current'),
        tomcatInfo('tomcat.sessions.active.max'),
        tomcatInfo('tomcat.sessions.rejected'),
        tomcatInfo('tomcat.global.sent'),
        tomcatInfo('tomcat.global.request.max'),
        tomcatInfo('tomcat.global.request'),
        // 2.1.3.RELEASE 无此API
        tomcatInfo('tomcat.servlet.request'),
        tomcatInfo('tomcat.servlet.request.max'),
        tomcatInfo('tomcat.threads.current'),
        tomcatInfo('tomcat.threads.config.max')
      ]).then((res) => {
        const tomcatInfo = []
